...
class BillClass:
    def __init__(self,root):
     ...
     #######product search
        ...
        btn_show_all=Button(ProductFrame2,text="Show All",command=self.fetch_data,font=("goudy old style",15),bg="#083531",fg="white",cursor="hand2").place(x=285,y=10,width=100,height=25)
    ...
    #####
       ...
       ###cart frame=====
        ...
        self.selfCartTable=ttk.Treeview(cart_frame,columns=("pid","name","price","qty"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set) 
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=X)
        scrollx.config(command=self.selfCartTable.xview)
        scrolly.config(command=self.selfCartTable.yview)
        self.selfCartTable.heading("pid",text="PID")
        self.selfCartTable.heading("name",text="Name")
        self.selfCartTable.heading("price",text="Price")
        self.selfCartTable.heading("qty",text="QTY")
        self.selfCartTable["show"]="headings"

        self.selfCartTable.column("pid",width=40)
        self.selfCartTable.column("name",width=100)
        self.selfCartTable.column("price",width=90)
        self.selfCartTable.column("qty",width=40)

        self.selfCartTable.pack(fill=BOTH,expand=1) 
        self.selfCartTable.bind("<ButtonRelease-1>",self.get_data_cart)
...
######billing button===
        ...
        self.lbl_amnt=Label(billMenuFrame,text="Bill Amout\n[0]",font=("goudy old style",15,"bold"),bg="#3f51b5",fg="white")
    ...
    def get_data(self,ev):
        cursors_row=self.product_Table.focus()
        content=self.product_Table.item(cursors_row)
        row=content['values']
    ...
    def get_data_cart(self,ev):
        ...
        cursors_row=self.selfCartTable.focus()
        content=self.selfCartTable.item(cursors_row)
        row=content['values']
        self.var_pid.set(row[0])
        self.var_pname.set(row[1])
        #self.var_qty.set(row[2])
    ...
    def show_cart(self): 
        try:
            self.selfCartTable.delete(*self.selfCartTable.get_children())
            for row in self.cart_list:
                self.selfCartTable.insert('',END,values=row)
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}",parent=self.root)
    ...
    def generate_bill(self):
        if self.var_cname.get()=='' or self.var_contact.get()=='': 
            ...
            messagebox.showerror("Error",f"Please Add product to the carrt!!!",parent=self.root)    
        else:
            ####bill top====
            self.bill_top()
            ####bill middle
    ...
    def bill_middle(self):
        ...
        try:
            ...
            # pid,name,price,qty,stock
                ...
                con.commit()
                self.cur.execute("INSERT INTO sales_history VALUES (?, ?, ?, ?, ?, ?, ?)", (str(self.invoice), self.var_cname.get(), self.var_contact.get(), str(time.strftime("%d/%m/%Y")), name, qty, self.net_pay))
                self.con.commit()
                con.close()
                self.fetch_data()
