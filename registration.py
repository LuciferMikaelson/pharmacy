...
class Register:
    def __init__(self,root):
        ...
        self.root.config(bg="#4d636d")

        ##===bg image==

       # self.bg=ImageTk.PhotoImage(file="images/bg22.jpg")
       # bg=Label(self.root,image=self.bg).place(x=250,y=0,relwidth=1,relheight=1)

        ##===left image==

        self.left=ImageTk.PhotoImage(file="images/download1.jpg")
        left=Label(self.root,image=self.left).place(x=300,y=250,width=400,height=500)

        ####Register frame==
        frame1=Frame(self.root,bg="white")
        frame1.place(x=700,y=250,width=700,height=500)
        ...
        self.btn_img=ImageTk.PhotoImage(file="images/rr1.png")
        btn_register=Button(frame1,image=self.btn_img,bd=0,cursor="hand2",command=self.register_data).place(x=50,y=400)
        btn_login=Button(self.root,text="Sign In",command=self.login_window,font=("times new roman",20),bd=0,cursor="hand2").place(x=400,y=600,width=180)
